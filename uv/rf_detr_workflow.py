...
WEIGHTS_PATH   = os.path.join(SCRIPT_DIR, "runs", "checkpoint_best_ema.pth")
TEST_IMG_DIR   = os.path.join(DATASET_DIR, "test", "images")
PRED_DIR       = os.path.join(SCRIPT_DIR, "predictions_v1test")
ZIP_PATH       = os.path.join(SCRIPT_DIR, "submission_rfdetr.zip")
    ...
    for split in ["train", "valid", "test"]:
        ...
        if not os.path.exists(ann_path):
            raise FileNotFoundError(
                f"Missing {ann_path}\nPlease run: python rf_detr_workflow.py --step convert"
            )
...
def step_predict():
    ...
    if not os.path.exists(WEIGHTS_PATH):
        raise FileNotFoundError(
            ...
            "Please run training first: python rf_detr_workflow.py --step train"
        )
...
def step_eval():
    ...
    if not os.path.isdir(pred_dir):
        raise FileNotFoundError(
            ...
            "Please run: python rf_detr_workflow.py --step predict"
        )
